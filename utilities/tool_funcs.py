...
def hitting_ratio(preds: torch.Tensor, truths: torch.Tensor, pred_topk: int, truth_topk: int, fixed=False):
    ...
    if fixed:
        preds = preds.clone()
        truths = truths.clone()
        preds.fill_diagonal_(float('inf'))
        truths.fill_diagonal_(float('inf'))
        _, preds_k_idx = torch.topk(preds, pred_topk, dim=1, largest=False)
        _, truths_k_idx = torch.topk(truths, truth_topk, dim=1, largest=False)
        ...
        return tp / (truth_topk * preds.shape[0])
    else:
