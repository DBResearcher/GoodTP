...
def laplacian_positional_encoding(g, adj, pos_enc_dim):
    ...
    available_eigenvectors = min(num_nodes - 1, pos_enc_dim)
    
    if available_eigenvectors > 0:
        lap_pos_feat = EigVec[:, 1 : available_eigenvectors + 1]
    else:
        lap_pos_feat = np.zeros((num_nodes, 0))
    
    # 填充到目标维度
    if lap_pos_feat.shape[1] < pos_enc_dim:
        padding = np.zeros((num_nodes, pos_enc_dim - lap_pos_feat.shape[1]))
        lap_pos_feat = np.concatenate([lap_pos_feat, padding], axis=1)
    
    g.ndata["lap_pos_feat"] = torch.from_numpy(lap_pos_feat).float()
    return g
...
def trajlist_to_trajgraph(traj_list, qtree, qtree_name2id, lon_range, lat_range):
    trajdict_list = _prepare(traj_list, qtree, qtree_name2id, lon_range, lat_range)
    trajgraph_list = _build_graph(trajdict_list)
    return trajgraph_list
...
def trajlist_to_trajgraph_parallel(traj_list, qtree, qtree_name2id, lon_range, lat_range, max_workers=40):
    """
    多线程版本的trajlist_to_trajgraph，保证输出顺序与输入一致
    """
    def process_single_traj(idx_traj):
        idx, traj = idx_traj
        trajdict = _prepare([traj], qtree, qtree_name2id, lon_range, lat_range)[0]
        g = _build_graph([trajdict])[0]
        return idx, g

    import os
    cpu_count = os.cpu_count() or 1
    limit = max(1, int(cpu_count * 0.3))
    effective_max_workers = min(max_workers, limit) if max_workers is not None else limit

    with ThreadPoolExecutor(max_workers=effective_max_workers) as executor:
        futures = [executor.submit(process_single_traj, (i, traj)) for i, traj in enumerate(traj_list)]
        results = [None] * len(traj_list)
