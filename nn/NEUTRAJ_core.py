...
class NeuTraj_Network(nn.Module):
    ...
    def single_embed(self, inputs_array, inputs_len_array):
        device = next(self.parameters()).device
        anchor_input = inputs_array #torch.tensor(inputs_array, dtype = torch.float, device = device, requires_grad = False)
        num_trajs = len(anchor_input)
        anchor_input_len = inputs_len_array
...
class RNNEncoder(nn.Module):
    ...
    def forward(self, inputs_a, initial_state=None):
        inputs, inputs_len = inputs_a  # inputs: [batch, time, input_size]
        ...
        for t in range(time_steps):
            ...
            outputs.append(out)  # each out: [batch, hidden]

        # 堆叠为 [time_steps, batch, hidden]，再转为 [batch, time_steps, hidden]
        outputs = torch.stack(outputs, dim=0).transpose(0, 1)  # [batch, time_steps, hidden]

        # inputs_len是每个序列的有效长度，取最后一个有效输出
        batch_indices = torch.arange(outputs.size(0), device=outputs.device)
        if not torch.is_tensor(inputs_len):
            inputs_len = torch.tensor(inputs_len, device=outputs.device)
        last_indices = inputs_len - 1  # [batch]
        last_outputs = outputs[batch_indices, last_indices, :]  # [batch, hidden]
