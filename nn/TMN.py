...
class SMNEncoder(Module):
    ...
    def f_single(self, src1, src1_len):
        input_a, input_len_a = src1, src1_len
        ...
        mask_a = (input_grid_a != 0)  # porto:220x149, 移除unsqueeze(-2)
        ...
        scores_a = torch.matmul(mlp_input_a, mlp_input_a.transpose(-2, -1))  # [batch, time_steps, time_steps]
            
        # 创建mask，将对角线设为0（避免自己关注自己）
        batch_size, time_steps = scores_a.shape[:2]
        diag_mask = torch.eye(time_steps, device=scores_a.device).unsqueeze(0).expand(batch_size, -1, -1)
        scores_a = scores_a.masked_fill(diag_mask == 1, -1e9)
            
        # 应用padding mask
        scores_a = scores_a.masked_fill(mask_a.unsqueeze(-1) == 0, -1e9)
        scores_a = scores_a.masked_fill(mask_a.unsqueeze(-2) == 0, -1e9)
            
        # 计算注意力权重
        p_attn_a = F.softmax(scores_a, dim=-1)
        p_attn_a = p_attn_a.masked_fill(mask_a.unsqueeze(-1) == 0, 0.0)
        p_attn_a = p_attn_a.masked_fill(mask_a.unsqueeze(-2) == 0, 0.0)
            
        # 加权求和
        attn_ab = p_attn_a.unsqueeze(-1).expand(-1, -1, -1, self.hidden_size // 2)
        sum_traj_b = mlp_input_a.unsqueeze(-2).expand(-1, -1, time_steps, -1).mul(attn_ab).sum(dim=-2)
        ...
        # mask_out_a = []
        # outputs_fa: [seq_len, batch, hidden]
        # input_len_a: list/1d tensor of length batch
        # 取每个batch的最后一个有效时刻的输出
        idx = torch.tensor(input_len_a, device=outputs_fa.device) - 1  # shape: [batch]
        batch_idx = torch.arange(outputs_fa.size(1), device=outputs_fa.device)
        fa_outputs = outputs_fa[idx, batch_idx, :]  # shape: [batch, hidden]
        # fa_outputs = torch.cat(mask_out_a, dim=0)
        return fa_outputs

    def f(self, src1, src1_len, src2=None, src2_len=None):

        if src2 is None:
            return self.f_single(src1, src1_len)
