...
class TrajGraphDataLoader:
    def __init__(self, traj_data, dis_matrix, phase, train_batch_size, eval_batch_size, 
                d_lap_pos, sample_num, data_features, num_workers, x_range, y_range, qtree, qtree_name2id) -> None:
        self.traj_data = traj_data
        ...
        if vir_node_layers == 1:
            ...
            for tt in tree_set:
                ...
                if self.qtree_name2id:
                    # 将每个结点对应的id进行存储
                    tree_id.append(self.qtree_name2id[hash(tt)])
                else:
    ...
    def _build_graph(self, trajdict_l_l):
        ...
        for trajdict_l in trajdict_l_l:
            ...
            for trajdict in trajdict_l:
                ...
                g = laplacian_positional_encoding_dense(g, adj, self.d_lap_pos)
