...
def learn_embeddings(walks, node2vec_args):
    ...
    model = Word2Vec(walks, vector_size=node2vec_args.dimensions, \
                    window=node2vec_args.window_size, min_count=0, \
                    sg=1, workers=node2vec_args.workers, epochs=node2vec_args.iter, \
                    compute_loss = True)
    ...
    # print(all_vectors[1])
    # print(all_vectors[-1])
    logging.info("[word2vec] train done. loss={:.4f}".format(model.get_latest_training_loss()))
    return all_vectors
