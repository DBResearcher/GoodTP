...
def train_node2vec(edge_index, d_model, device):
    # edge_index: tensor [2, n]
    logging.info("[node2vec] start.")

    model = Node2Vec(torch.tensor(edge_index, dtype = torch.long).T, 
    ...
    epoch_total = 10 # we use 100 previously
    ...
    for epoch in range(epoch_total):
        ...
        if loss < epoch_train_loss_best:
            ...
            epoch_worse_count = 0
            # save_checkpoint()
            save_embeddings = model.embedding.weight.data.detach().clone()
        else:
            epoch_worse_count += 1
