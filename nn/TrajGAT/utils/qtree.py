...
def get_qtree_feat(root):
    ...
    for index, node in enumerate(virtual_node):
        # if hash(node) in vir_node2id:
        #     old = virtual_node[vir_node2id[hash(node)]]
        #     logging.info('collison in hash values. old: {}, {}, {}. new: {}, {}, {}'.format( \
        #                 old.center, old.height, old.width, node.center, node.height, node.width))
...
class _QuadTree(object):
    ...
    def __hash__(self):
        ...
        # return hash( str(self.center[0])+str(self.center[1]) ) # dont use hash function. it has collison
        # return int(self.center[0] * 1e60 + self.center[1] * 1e20)
